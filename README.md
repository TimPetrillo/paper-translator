...
Paper Translator 是一个面向英文学术论文网页的开源中文翻译扩展。它保留网页原有 DOM 结构，支持双语显示与原位替换，并允许用户连接自己的 OpenAI Compatible Chat Completions API。项目基于 Manifest V3、TypeScript、React 和 Vite，可用于 Chrome 与 Edge。

> 当前版本：`0.1.0`。项目不提供公共翻译代理，不收集用户数据，也不会把 API Key 上传到项目作者的服务器。
...
- 使用通用 Chat Completions 请求格式，不绑定模型或服务商
- API 配置使用 `chrome.storage.local` 保存在本机浏览器
- 无遥测、无广告、无账户系统

## 截图

以下图片是截图占位图。发布正式版本前，可用真实界面截图替换 `docs/images/` 中的文件。

![Popup 截图占位](docs/images/popup-placeholder.svg)

![论文双语翻译截图占位](docs/images/bilingual-placeholder.svg)

## 支持的 API

扩展请求用户配置的 `POST {Base URL}/chat/completions`，使用 Bearer API Key，并读取 `choices[0].message.content`。因此可兼容：

- OpenAI / ChatGPT API
- DeepSeek API
- Qwen OpenAI 兼容接口
- GLM OpenAI 兼容接口
- 其他遵循同一请求/响应结构的自建或第三方服务
...
1. 打开扩展 Popup。
2. 填写 API Key、Base URL 和 Model Name。
...
若服务商给出的完整地址是 `https://api.example.com/v1/chat/completions`，Base URL 可填写完整地址；插件不会重复追加路径。

## 工作原理

```text
Popup 保存配置 → Content Script 提取可见英文文本
                → 按文本节点/句子分块与并发调度
...
确认 Base URL 包含服务商要求的版本路径。扩展默认追加 `/chat/completions`；也可直接填写完整 Chat Completions 地址。
...
不会上传到本项目。API Key 会作为 Bearer 凭据发送到你填写的 API 地址，这是完成请求所必需的。请只使用可信的 HTTPS 端点。
...
├── docs/images/             # README 截图/占位图
├── public/icons/            # 扩展图标
├── scripts/                 # 发布 ZIP 脚本
├── src/
│   ├── api/                 # OpenAI Compatible API 客户端
